...
---
## Problem Statement
 Due to the variable nature of renewable energies (i.e.- energies reliant on conditions such as cloudiness for solar or windiness for wind energy), the integration of solar energy generation sites with the larger electrical grids can be extremely complicated. One way of easing this burden is accurately reporting and predicting power generation from these variable generation sites. Therefore, using the solar generation output of a specific solar farm site, along with the weather data, can one predict the solar generation output of that specific site within a mean-absolute-error score of less than 10% of max output?

 Furthermore can the same model architecture found optimized for a specific site be used to predict the generation of another site or even another full campus?
...
## Whom This Concerns
 ...
 Also, if the model for a single site can be used for prediction for other site or even whole campuses, then the modeling process does not need to be iterated through for each location, saving time and resources.
...
Based on the wide variety of analysis and modeling conducted, the model found would be able to predict the solar generation output with a MAE score of less than 10% of the maximum solar generaiton output (0.6 kWh in the case of Campus 5).   
...
However, the variability in the output relies on many things such as the component's functionality and weather patterns. Consequently, future research specifically on the weather data's effect on energy generation on these sites is recommended.
